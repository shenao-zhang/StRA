...
class LocalAttn(nn.Module):
    ...
    def forward(self, x):
        ...
        mask_neighbor = self.mask_conv(x)  # b,g*(1+k*k),h,w
        neighbor_feature = mask_neighbor[:, :self.groups, :, :]  # b,g,h,w
        mask = mask_neighbor[:, self.groups:, :, :]  # b,g*k*k,h,w
        neighbor_feature = self.unfold(neighbor_feature)  # b,g*k*k,h,w
        neighbor_feature = neighbor_feature.view(b, self.groups, self.kernel_size * self.kernel_size, h, w)  # b,g,k*k,h,w
        mask = mask.view(b, self.groups, self.kernel_size * self.kernel_size, h, w)  # b,g,k*k,h,w
        mask = mask + neighbor_feature  # b,g,k*k,h,w
        mask = F.softmax(mask, dim=2)
        mask = mask.unsqueeze(2)   # b,g,1,k*k,h,w

        value = self.value_conv(x)
        value = self.unfold(value)  # b,c*k*k,h,w
        value = value.view(b, self.groups, self.out_channels // self.groups, -1, h, w)  # b,g,out/g,k*k,h,w

        out = mask * value  # b,g,out/g,k*k,h,w
        out = torch.sum(out, dim=3, keepdim=False)  # b,g,out/g,h,w
        out = out.view(b, -1, h, w)
        out, fnorm = self.moe(out)
        global fnorm_loss
        fnorm = F.pad(input=fnorm, pad=(16-fnorm.size(0), 0))  # to avoid error when testing
        fnorm = fnorm.cpu()
        fnorm_loss = fnorm_loss + fnorm
        return out
